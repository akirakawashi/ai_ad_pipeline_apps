...
class User(SQLModel, table=True):
    ...
    Обещанная авторизация приехала, и справочник стал таблицей пользователей
    ровно так, как было задумано: связи не тронуты, добавились колонки. Личность
    приходит из Keycloak, запись заводится сама при первом входе.

    Отсюда три следствия, каждое из которых меняет прежнее решение:

    - **Ключ связи с Keycloak — `keycloak_subject`, не логин и не ФИО.** `sub`
      закреплён за человеком навсегда, а доменный логин меняется вместе с
      фамилией: после свадьбы сотрудница приехала бы как новая, потеряв свою
      историю заданий и съёмок.
    - **С `full_name` снят UNIQUE.** Он защищал от дублей, пока людей заводили
      руками из выпадашки, и там был к месту. Теперь имя приходит из AD, а два
      настоящих полных тёзки в компании — обычное дело: уникальность отказала бы
      второму из них во входе. Проверка на дубли переехала туда, где остался
      ручной ввод, — в справочник админ-панели.
    - **`groups_raw` — диагностика, а не права.** В токене приезжает вся
      оргструктура: отделы, рассылки, доступы к шарам. Права считаются из неё по
      белому списку и лежат в `permissions`; сырой список хранится, чтобы на
      вопрос «почему Петров не админ» смотреть, что реально пришло, а не гадать.
      Решать по нему доступ нельзя — белый список на то и белый.

    Записи, заведённые руками до перехода на SSO, остаются с пустым
    `keycloak_subject`: они по-прежнему числятся авторами прошлых заданий и
    съёмок, но войти под ними нельзя. Сшивка такой записи с доменной учёткой —
    отдельная задача, здесь для неё только место.
    """
    ...
    # Индекс без уникальности: тёзки в AD — норма, см. докстринг класса.
    full_name: str = Field(
        sa_column=Column(
            String(255),
            index=True,
    ...
    # `sub` из Keycloak — единственный стабильный идентификатор человека.
    # NULL у записей, заведённых руками до SSO: под ними нельзя войти, но их
    # история сохраняется.
    keycloak_subject: str | None = Field(
        default=None,
        sa_column=Column(
            String(64),
            unique=True,
    ...
    # Доменный логин (`preferred_username`). Только для показа и поиска: меняется
    # при смене фамилии, связывать по нему нельзя.
    ...
    # Сырые группы токена как есть, полным путём. Персданные и оргструктура —
    # наружу по HTTP не отдаются, только в админ-панель.
    groups_raw: list[str] | None = Field(
        default=None,
        sa_column=Column(JSONB(none_as_null=True), nullable=True),
    )
    # Права, посчитанные из групп при последнем входе. Зеркало, а не источник
    # правды: доступ решается по группам текущего токена, а это — чтобы показать
    # в справочнике, кто есть кто, не дёргая Keycloak.
    permissions: list[str] = Field(
        default_factory=list,
        sa_column=Column(JSONB(none_as_null=True), nullable=False, default=list),
    )
...
class UserSession(SQLModel, table=True):
    """Сессия входа: браузер держит непрозрачный ключ, всё остальное — здесь.

    Токен Keycloak не хранится вовсе. Он нужен ровно один раз — на callback,
    чтобы узнать, кто пришёл и в каких он группах, — а дальше приложение никуда
    от имени человека не ходит. Хранить его значило бы держать живой пропуск на
    диске без единого сценария, где он пригодится.

    Почему серверная запись, а не подписанная cookie с содержимым внутри:

    - **Размер.** В корпоративном AD человек состоит в десятках групп, и токен
      с ними распухает до килобайтов. Cookie ограничена четырьмя, заголовок
      `Authorization` — лимитами nginx и uvicorn. Обе границы упираются не в
      среднего сотрудника, а в самого «тяжёлого», то есть ломается у одного
      человека из ста и невоспроизводимо.
    - **Отзыв.** Токен живёт 8 часов и не отзывается: сотрудника отключили в AD
      в девять утра, а его токен действителен до пяти вечера. Строка в базе —
      единственное место, где такого человека можно выкинуть немедленно.

    `expires_at` берётся из `exp` токена, а не назначается своим сроком: сессия
    приложения не должна переживать пропуск, по которому она выдана.

    В базе лежит SHA-256 ключа, а не сам ключ, — по той же причине, по которой
    не хранят пароли: дамп таблицы не должен давать возможности войти.
    """

    __tablename__ = "user_sessions"

    user_sessions_id: str = Field(
        default_factory=uuid_string,
        sa_column=Column(
            String(36),
            primary_key=True,
    ...
    # CASCADE здесь, в отличие от остальных связей с `users`, намеренно: сессия —
    # не история, а пропуск. Пережившая своего человека сессия была бы дырой.
    # Без индекса: поиск идёт по `token_hash`, а срок только доусловие в том же
    # запросе. Отдельной выборки по сроку нет — истёкшие строки не убираются
    # вовсе. При двух пользователях это пара строк в сутки; появится уборка —
    # индекс вернётся вместе с ней.
