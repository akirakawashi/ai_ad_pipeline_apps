...
class AuthModeResponse(ApiModel):
    """Каким способом входят и работает ли это.

    `keycloak` — `true` для доменного входа, `false` для логина и пароля.

    `ready` — можно ли этим способом воспользоваться прямо сейчас. Отдельно от
    режима, потому что состояние «выбран Keycloak, но реквизиты не заполнены»
    существует: сервис в нём поднимается и работает, а войти нельзя. Без этого
    флага интерфейс нарисовал бы кнопку «Войти», ведущую в 503, — а человек
    решил бы, что сломана авторизация.

    Адреса Keycloak и client_id наружу не отдаются: браузер к нему сам не ходит,
    его уводит `/auth/login`.
    """
