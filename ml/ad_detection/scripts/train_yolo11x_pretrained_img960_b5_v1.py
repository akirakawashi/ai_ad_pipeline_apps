...
DATA_YAML = PROJECT_ROOT / "ml/data/detection/yolo/ad_surface_full_v1/data.yaml"
MODEL_SOURCE = PROJECT_ROOT / "models/detection/pretrained/yolo11x.pt"
RUN_PROJECT = PROJECT_ROOT / "ml/runs/detection/detect/ad_surface_full_v1"
RUN_NAME = "yolo11x_pretrained_img960_b5_full_v1"
EXPORT_DIR = PROJECT_ROOT / "models/detection/trained/yolo11x_pretrained_img960_b5_full_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train YOLO11x pretrained quality run on ad_surface_full_v1.")
    ...
    parser.add_argument("--batch", type=parse_batch, default=5)
...
def main() -> None:
    ...
    config = {
        "hypothesis": "YOLO11x pretrained, lower image size to keep batch=5 on limited VRAM",
        "data": str(data_yaml),
    ...
    if not model_source.exists():
        raise FileNotFoundError(
            f"{model_source} not found. Put YOLO11x pretrained weights at this path before running."
        )
